  ...
  try {
    ...
    if (isCmd && msgFilter.isFiltered(from) && !isGroupMsg && !isOwnerBot) {
      return (
        ...
        client.reply(from, "Mohon untuk tidak melakukan spam", id)
      );
    }
    if (isCmd && msgFilter.isFiltered(from) && isGroupMsg && !isOwnerBot) {
      return (
      ...
      client.simulateTyping(chat.id, true).then(async () => {
        switch (command) {
            ...
            await client
              ...
              .then(() =>
                isGroupMsg && isGroupAdmins
                  ? client.sendText(
                      from,
                      `Menu Admin Grup: *${prefix}menuadmin*`
                    )
                  : null
              );
            break;
          ...
          case "join":
            ...
            if (isOwnerBot) {
              await client.joinGroupViaLink(linkgrup).then(async () => {
                await client.sendText(from, "Berhasil join grup via link!");
                await client.sendText(
                  chekgrup.id,
                  `Hai minna~, Im CRazyz BOT. To find out the commands on this Bot type ${prefix}menu`
                );
              });
            } else {
          ...
          case "stimg":
            if (quotedMsg && quotedMsg.type == "sticker") {
              ...
              return client.reply(
                from,
                `Format salah, silahkan tag/reply sticker yang ingin dijadikan gambar!`,
                id
              );
            break;
          ...
          case "stiker":
            if ((isMedia || isQuotedImage) && args.length === 0) {
              ...
              await writeFile(outFilest, imageBase64, "binary")
          ...
          case "tes":
            client.reply(from, `Tidak ada fitur untuk dites`, id);
            break;
          ...
          case "stikergif":
            if (isMedia || isQuotedVideo) {
              if (
                ...
                client.reply(
                  from,
                  "[WAIT] Sedang diproses⏳ silakan tunggu ± 1 min!",
                  id
                );
                var filename = `./media/stickergif.${mimetype.split("/")[1]}`;
                await fs.writeFileSync(filename, mediaData);
                client.sendMp4AsSticker(from, filename, stickerMetadata);
              } else {
                client.reply(
                  from,
                  `[❗] Kirim video dengan caption *${prefix}stickergif* max 10 sec!`,
                  id
                );
              }
            } else {
              client.reply(
                from,
                `[❗] Kirim video dengan caption *${prefix}stickergif*`,
                id
              );
            }
            break;
          ...
          case "qrcode":
            if (args.length == 0)
              return client.reply(
                from,
                `Untuk membuat kode qr, ketik ${prefix}qrcode <kata>\nContoh:  ${prefix}qrcode nama saya client`,
                id
              );
            ...
            console.log(kata);
            rugaapi.qrcode(kata, 500).then(async (res) => {
          ...
          case "jsolat":
            if (args.length === 0)
              return client.reply(
                from,
                `ketik *${prefix}jsholat <nama kabupaten>* untuk melihat jadwal sholat\nContoh: *${prefix}jsholat sleman*\nUntuk melihat daftar daerah, ketik *${prefix}jsholat daerah*`,
                id
              );
            if (args[0] == "daerah") {
              var datad = await axios.get(
                  ...
                  await download(link, dir, function (err) {
                    if (err) {
                      ...
                      client
                        ...
                        .then(
                          console.log(
                            `Audio Processed for ${processTime(
                              t,
                              moment()
                            )} Second`
                          )
          ...
          case "pantun":
            fetch(
              ...
              .then((body) => {
                ...
                client.reply(
                  from,
                  " " + randompantun.replace(/aruga-line/g, "\n"),
                  id
                );
              })
              .catch(() => {
          ...
          case "quotes":
            ...
            await client.reply(from, quotex, id).catch(() => {
              client.reply(from, "Ada yang Error!", id);
            });
            break;
          ...
          case "memes":
            ...
            client.sendFileFromUrl(from, randmeme, "", "", id).catch(() => {
              client.reply(from, "Ada yang Error!", id);
            });
            break;
          ...
          case "dewabatch":
            if (args.length == 0)
              return client.reply(
                from,
                `Untuk mencari anime batch dari Dewa Batch, ketik ${prefix}animebatch judul\n\nContoh: ${prefix}animebatch naruto`,
                id
              );
          ...
          case "images":
            ...
            await client
              .sendFileFromUrl(from, hasilwall, "", "", id)
              .catch(() => {
                client.reply(from, "Ada yang Error!", id);
              });
            break;
          ...
          case "nekopoi":
            if (isGroupMsg) {
              client.reply(
                from,
                "Untuk Fitur Nekopoi Silahkan Lakukan di Private Message",
                id
              );
            } else {
              var data = await axios.get(
                "https://arugaz.my.id/api/anime/nekopoi/random"
              );
          ...
          case "cuaca":
            ...
            await client.reply(from, cuacap, id).catch(() => {
              client.reply(from, "Ada yang Error!", id);
            });
            break;
          ...
          case "play": //silahkan kalian custom sendiri jika ada yang ingin diubah
            ...
            axios
              .get(
                `http://api.arugaz.my.id/api/media/ytsearch?query=${body.slice(
                  6
                )}`
              )
              .then(async (res) => {
          ...
          case "say":
            if (!isQuotedChat && args.length !== 0) {
              ...
              if (dataText === "")
                return client.reply(from, "Apa teksnya syg..", id);
              try {
          ...
          case "shortlink":
            if (args.length == 0)
              return client.reply(from, `ketik ${prefix}shortlink <url>`, id);
            if (!isUrl(args[0]))
              return client.reply(
                from,
            ...
            await client.sendText(from, shortlink).catch(() => {
              client.reply(from, "Ada yang Error!", id);
            });
            break;
          ...
          case "hilih":
            if (args.length !== 0) {
              rugaapi.hilihfont(body.slice(7)).then(async (res) => {
                await client.reply(from, `${res}`, id);
              });
            } else if (isQuotedChat && args.length === 0) {
              rugaapi
                .hilihfont(quotedMsgObj.content.toString())
                .then(async (res) => {
          ...
          case "skripsi":
            let randomSkripsi =
              skripsi[Math.floor(Math.random() * skripsi.length)];
          ...
          case "apakah":
            ...
            if (args.length === 0)
              result =
                "Apakah apa woy yang jelas dong! Misalnya, apakah aku ganteng?";
            else {
          ...
          case "promote":
            ...
            if (mentionedJidList.length !== 1)
              return client.reply(
                from,
                "Maaf, hanya bisa mempromote 1 user",
                id
              );
          ...
          case "demote":
            ...
            if (mentionedJidList.length !== 1)
              return client.reply(
                from,
                "Maaf, hanya bisa mendemote 1 user",
                id
              );
            if (!groupAdmins.includes(mentionedJidList[0]))
              return await client.reply(
          ...
          case "sija":
            ...
            if (args[0] == "add") {
              ...
              client.reply(
                from,
                "Nomor sudah ditambahkan ke daftar kontak SIJA",
                id
              );
            } else if (args[0] == "del") {
              for (let i = 1; i < args.length; i++) {
              ...
              client.reply(
                from,
                "Nomor sudah dihapus dari daftar kontak SIJA",
                id
              );
            } else {
          ...
          case "welcome":
            ...
            if (args.length !== 1)
              return client.reply(
                from,
                `Membuat BOT menyapa member yang baru join kedalam group chat!\n\nPenggunaan:\n${prefix}welcome on --aktifkan\n${prefix}welcome off --nonaktifkan`,
                id
              );
            if (args[0] == "on") {
              welcome.push(chatId);
